...
def _buy_power(c: KISClient, sym: str, price: int) -> tuple[int, int]:
    """(미수없는 매수가능수량, 주문가능현금) — 통합증거금(해외 포함) 반영.
    get_balance의 ord_psbl_cash는 이 계좌에서 만성적으로 0이라 매수가능조회를 쓴다."""
    ps = c.inquire_psbl_order(sym, price, order_division="01", include_overseas="Y")
    out = ps.get("output") or {}
    if isinstance(out, list):
        out = out[0] if out else {}
    nrcvb_qty = int(float(out.get("nrcvb_buy_qty", 0) or 0))   # 미수 없는(현금) 매수가능수량
    cash = int(float(out.get("ord_psbl_cash", 0) or 0))
    return nrcvb_qty, cash
...
def main() -> None:
    ...
    # 2) 인버스 매수 사이즈 계산 (올인 금지: 목표 상한 & 미수없는 매수가능 한도)
    time.sleep(1.0)
    ...
    if price <= 0:
        ...
        return
    nrcvb_qty, cash = _buy_power(c, INV_SYMBOL, price)
    target_qty = TARGET_KRW // price
    qty = min(nrcvb_qty, target_qty)
    print(f"[2] 인버스 매수 계산: 현재가 {price:,} | 미수없는매수가능 {nrcvb_qty}주 | 주문가능현금 {cash:,} "
          f"| 목표상한 {TARGET_KRW:,}({target_qty}주) → {qty}주")
    if qty < 1:
        print("!! 미수없는 매수가능수량 0 — 인버스 매수 불가. 현재 플랫 상태로 종료 !!")
        return
